...
client.on("messageCreate", (message) => {
  ...
  if (
    message.channel.id === "786715096202215494" &&
    message.content.toLowerCase().includes("aaron")
  ) {
    // #aaron
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (
    apiKey === undefined &&
    (canvasCommands.includes(interaction?.commandName) ||
      canvasCommands.includes(interaction?.customId?.replace(/\[[^\]]]/g, "")))
  ) {
    let embed = new Discord.MessageEmbed()
      .setColor(config.bot.color)
      .setTitle("You Must Register!")
